    ...
    useEffect(() => {
        const fetchResources = async () => {
            try {
                ...
                const sitesResponse = await getAuth(`/pms/sites.json${token ? `?token=${token}` : ''}`);
                const sitesData = sitesResponse?.sites || sitesResponse || [];
                setProperties(Array.isArray(sitesData) ? sitesData : []);

                // Fetch Vendors
                const vendorsResponse = await getAuth('/vendors.json');
                const vendorsData = vendorsResponse?.vendors || vendorsResponse?.amc_vendors || vendorsResponse || [];
                setVendors(Array.isArray(vendorsData) ? vendorsData : []);

                // Fetch Expense Categories
                const categoriesResponse = await getAuth('/expense_categories.json');
                const categoriesData = categoriesResponse?.expense_categories || categoriesResponse || [];
                        ...
                        <CardContent className="p-6 space-y-6">
                            ...
                            <div className="flex items-center justify-between p-4 border border-gray-200 rounded-lg bg-gray-50">
                                <div className="space-y-0.5">
                                    ...
                                    <p className="text-sm text-gray-500">Enable if this is a repeating expense (e.g. monthly rent, utilities)</p>
                                </div>
                                <Switch
